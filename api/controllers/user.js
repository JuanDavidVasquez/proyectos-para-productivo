...
function home(req, res) {
  res.status(200).send({
    message: "Accion de home del servidor de NodeJS",
  });
}
  ...
  if (
    ...
    User.find({
      ...
      if (users && users.length >= 1) {
        ...
        //Cifrar la password y guardar datos
            ...
            bcrypt.compare(password, user.password, (err, check)=>{
                if(check){
                    ...
                    if(params.gettoken){
                        ...
                        user.password = undefined;//eliminar propiedad de los datos devueltos
...
function getUsers(req,res){
	...
	User.find().sort('_id').exec((err,users)=>{
		...
		if(!users) return res.status(404).send({message: 'No exisen usuarios para mostrar'});
...
function uploadImage(req, res){
	...
	//	var file_split = file_path.split('\\');  local
